...
export const NodeCard: React.FC<NodeCardProps> = ({ node,colorTheme }) => {
  const colors = OrgChartUtils.getNodeColors(node.nivel_jerarquico, colorTheme);
  ...
  return (
    ...
    `}>
      ...
      <p className="text-xs opacity-90 mt-1">Nivel Jerarquico: {node.nivel_jerarquico}</p>
    </div>
  );
};
