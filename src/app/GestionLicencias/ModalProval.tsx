...
export const ApprovalModal = ({ request, supervisores, onClose, onManage }) => {
  ...
  const handleReject = () => {
    if (!observacion.trim()) {
      alert("Debe agregar una observación para rechazar.");
      return;
    }
  ...
  return (
    <Modal
      show={true}
      onClose={onClose}
      title={`Revisión de Solicitud de ${request.solicitanteNombre}`}
    >
      <div className="space-y-4">
          ...
          <div className="mt-2 space-y-3">
            <div>
              <label className="font-semibold block mb-1">
                Aprobar y derivar a (opcional):
              </label>
              <select
                value={siguienteSupervisorId}
                onChange={(e) => setSiguienteSupervisorId(e.target.value)}
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          <Button
            ...
            variant="danger"
            disabled={!!siguienteSupervisorId || !observacion}
          >
            Rechazar
          </Button>
          <Button
